...
struct Snare : Module {
  ...
  enum LightId { SNARE_LIGHT, LIGHTS_LEN };
  ...
  unsigned int sampleLengths[16] = {
      Snare1_len,  Snare2_len,  Snare3_len,  Snare4_len,
      Snare5_len,  Snare6_len,  Snare7_len,  Snare8_len,
      Snare9_len,  Snare10_len, Snare11_len, Snare12_len,
  ...
  void process(const ProcessArgs &args) override {
    ...
    // Decay Snare light
    SnareLightBrightness =
        std::max(0.f, SnareLightBrightness - args.sampleTime * 10.f);
    lights[SNARE_LIGHT].setBrightnessSmooth(SnareLightBrightness,
                                            args.sampleTime);
...
struct SnareWidget : ModuleWidget {
  SnareWidget(Snare *module) {
    setModule(module);
    setPanel(
        createPanel(asset::plugin(pluginInstance, "res/panels/Snare.svg")));
