...
struct SeaArr : Module {
  ...
  Voice cowVoice, congaVoice, bongoLVoice, bongoHVoice, tambVoice, guiroVoice,
      cymVoice;
  ...
  SeaArr() {
    ...
    configSwitch(HATMETALPUSH_PARAM, 0.f, 1.f, 0.f, "Hat Metal Trig",
                 {"Off", "On"});
    ...
    configSwitch(BONGOHPUSH_PARAM, 0.f, 1.f, 0.f, "Bongo Hi Trig",
                 {"Off", "On"});
    configSwitch(TAMPUSH_PARAM, 0.f, 1.f, 0.f, "Tambourine Trig",
                 {"Off", "On"});
    ...
    snareVoice =
        createVoice(SASnare, sizeof(SASnare), SNAREOUT_OUTPUT, SNARE_LIGHT);
    hatVoice = createVoice(SAHiHat, sizeof(SAHiHat), HATOUT_OUTPUT, HAT_LIGHT);
    hatMetalVoice = createVoice(SAHiHatMetal, sizeof(SAHiHatMetal),
                                HATMETALOUT_OUTPUT, HATMETAL_LIGHT);
    rimVoice = createVoice(SARim, sizeof(SARim), RIMOUT_OUTPUT, RIM_LIGHT);
    cowVoice =
        createVoice(SACowbell, sizeof(SACowbell), COWOUT_OUTPUT, COW_LIGHT);
    congaVoice =
        createVoice(SACongaL, sizeof(SACongaL), CONGAOUT_OUTPUT, CONGA_LIGHT);
    bongoLVoice =
        createVoice(SABongoL, sizeof(SABongoL), BONGOLOUT_OUTPUT, BONGOL_LIGHT);
    bongoHVoice =
        createVoice(SABongoH, sizeof(SABongoH), BONGOHOUT_OUTPUT, BONGOH_LIGHT);
    tambVoice = createVoice(SATamb, sizeof(SATamb), TAMOUT_OUTPUT, TAM_LIGHT);
    guiroVoice =
        createVoice(SAGuiro, sizeof(SAGuiro), GUIROOUT_OUTPUT, GUIRO_LIGHT);
  ...
  void process(const ProcessArgs &args) override {
    ...
    processVoice(args, hatVoice, HATTRIGIN_INPUT, HATPUSH_PARAM, speed,
                 lengthRatio);
    processVoice(args, hatMetalVoice, HATMETALTRIGIN_INPUT, HATMETALPUSH_PARAM,
                 speed, lengthRatio);
    processVoice(args, rimVoice, RIMTRIGIN_INPUT, RIMPUSH_PARAM, speed,
                 lengthRatio);
    processVoice(args, cowVoice, COWTRIGIN_INPUT, COWPUSH_PARAM, speed,
                 lengthRatio);
    processVoice(args, congaVoice, CONGATRIGIN_INPUT, CONGAPUSH_PARAM, speed,
                 lengthRatio);
    ...
    processVoice(args, bongoHVoice, BONGOHTRIGIN_INPUT, BONGOHPUSH_PARAM, speed,
                 lengthRatio);
    processVoice(args, tambVoice, TAMTRIGIN_INPUT, TAMPUSH_PARAM, speed,
                 lengthRatio);
    processVoice(args, guiroVoice, GUIROTRIGIN_INPUT, GUIROPUSH_PARAM, speed,
                 lengthRatio);
    ...
    float busSum = 0.f;
    Voice *allVoices[] = {&kickVoice,     &snareVoice,  &hatVoice,
                          &hatMetalVoice, &rimVoice,    &cowVoice,
                          &congaVoice,    &bongoLVoice, &bongoHVoice,
                          &tambVoice,     &guiroVoice,  &cymVoice};
...
struct SeaArrWidget : ModuleWidget {
  SeaArrWidget(SeaArr *module) {
    setModule(module);
    setPanel(
        createPanel(asset::plugin(pluginInstance, "res/panels/SeaArr.svg")));
