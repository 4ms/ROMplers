...
struct Tom : Module {
  ...
  enum LightId { TOM_LIGHT, LIGHTS_LEN };
  ...
  void process(const ProcessArgs &args) override {
    ...
    if (TomLightBrightness < 0.f)
      TomLightBrightness = 0.f;
    ...
    if (playing && currentSample) {
      ...
      if ((int)samplePos >= sampleLengthSamples) {
        ...
        if (inputs[DECAYCVIN_INPUT].isConnected())
          cvV = std::clamp(inputs[DECAYCVIN_INPUT].getVoltage(), -10.f, 10.f) *
                0.5f;
        float decayParam = std::clamp(knobV + cvV, 0.f, 5.f) / 5.f;

        const float minDecayTime = 0.005f;
        const float maxDecayTime =
            (float)sampleLengthSamples / SAMPLE_SAMPLE_RATE;
        float decayTime =
