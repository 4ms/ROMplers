...
struct AyysKing : Module {
  ...
  AyysKing() {
    ...
    for (int i = 0; i < 10; i++) {
      configSwitch(KICKPUSH_PARAM + i, 0.f, 1.f, 0.f,
                   string::f("%s Trig", labels[i]), {"Off", "On"});
    ...
    voices[1] =
        createVoice(AKSnare, sizeof(AKSnare), SNARE1OUT_OUTPUT, SNARE1_LIGHT);
    voices[2] =
        createVoice(AKSnare2, sizeof(AKSnare2), SNARE2OUT_OUTPUT, SNARE2_LIGHT);
    voices[3] = createVoice(AKClosedHiHat, sizeof(AKClosedHiHat),
                            CLOSEDHATOUT_OUTPUT, CLOSEDHAT_LIGHT);
    voices[4] = createVoice(AKOpenHiHat, sizeof(AKOpenHiHat), OPENHATOUT_OUTPUT,
                            OPENHAT_LIGHT);
    voices[5] =
        createVoice(AKBongo, sizeof(AKBongo), BONGO1OUT_OUTPUT, BONGO1_LIGHT);
    voices[6] =
        createVoice(AKBongo2, sizeof(AKBongo2), BONGO2OUT_OUTPUT, BONGO2_LIGHT);
    voices[7] =
        createVoice(AKBongo3, sizeof(AKBongo3), BONGO3OUT_OUTPUT, BONGO3_LIGHT);
    voices[8] =
        createVoice(AKWood, sizeof(AKWood), CLAVEOUT_OUTPUT, CLAVE_LIGHT);
    voices[9] =
        createVoice(AKCrash, sizeof(AKCrash), CYMOUT_OUTPUT, CYMBAL_LIGHT);
  }

  Voice createVoice(const unsigned char *data, size_t size, int outId,
                    int lightId) {
    Voice v;
    v.rawData = data;
  ...
  void processVoice(const ProcessArgs &args, Voice &voice, int trigInputId,
                    int pushParamId, float speed, int maxSamples) {
    ...
    const bool trigger = trigRise || btnRise;
  ...
  void process(const ProcessArgs &args) override {
    // --- Speed ---
    float speedKnobV =
        (params[SPEED_PARAM].getValue() + 1.f) * 2.5f; // -1..1 → 0–5V
    float speedCVV = 0.f;
    if (inputs[SPEEDCVIN_INPUT].isConnected())
      speedCVV =
          std::clamp(inputs[SPEEDCVIN_INPUT].getVoltage(), -10.f, 10.f) * 0.5f;
    float speedNorm = std::clamp(speedKnobV + speedCVV, 0.f, 5.f) / 5.f;
    float speed = SPEED_LOW + speedNorm * (SPEED_HIGH - SPEED_LOW);

    // --- Length ---
    float lengthKnobV = params[LENGTH_PARAM].getValue() * 5.f;
    float lengthCVV = 0.f;
    ...
    // --- Process each voice ---
    for (int i = 0; i < 10; i++) {
      int voiceMaxSamples = int(voices[i].sampleLength * lengthRatio);
      processVoice(args, voices[i], KICKTRIGIN_INPUT + i, KICKPUSH_PARAM + i,
                   speed, voiceMaxSamples);
    }
...
struct AyysKingWidget : ModuleWidget {
  AyysKingWidget(AyysKing *module) {
    setModule(module);
    setPanel(
        createPanel(asset::plugin(pluginInstance, "res/panels/AyysKing.svg")));
    ...
    addParam(createParamCentered<Davies1900hBlack>(
        mm2px(Vec(11.24, 15.501)), module, AyysKing::LENGTH_PARAM));
    addParam(createParamCentered<Davies1900hBlack>(
        mm2px(Vec(31.249, 15.501)), module, AyysKing::SPEED_PARAM));
    addParam(createParamCentered<Davies1900hBlack>(
        mm2px(Vec(49.731, 15.501)), module, AyysKing::MAINVOL_PARAM));

    addParam(createParamCentered<LEDBezel>(mm2px(Vec(6.999, 42.002)), module,
                                           AyysKing::KICKPUSH_PARAM));
    addChild(createLightCentered<LEDBezelLight<WhiteLight>>(
        mm2px(Vec(6.999, 42.002)), module, AyysKing::KICK_LIGHT));

    addParam(createParamCentered<LEDBezel>(mm2px(Vec(18.75, 42.002)), module,
                                           AyysKing::SNARE1PUSH_PARAM));
    addChild(createLightCentered<LEDBezelLight<WhiteLight>>(
        mm2px(Vec(18.75, 42.002)), module, AyysKing::SNARE1_LIGHT));

    addParam(createParamCentered<LEDBezel>(mm2px(Vec(30.501, 42.002)), module,
                                           AyysKing::SNARE2PUSH_PARAM));
    addChild(createLightCentered<LEDBezelLight<WhiteLight>>(
        mm2px(Vec(30.501, 42.002)), module, AyysKing::SNARE2_LIGHT));

    addParam(createParamCentered<LEDBezel>(mm2px(Vec(42.249, 42.002)), module,
                                           AyysKing::CLOSEDHATPUSH_PARAM));
    addChild(createLightCentered<LEDBezelLight<WhiteLight>>(
        mm2px(Vec(42.249, 42.002)), module, AyysKing::CLOSEDHAT_LIGHT));

    addParam(createParamCentered<LEDBezel>(mm2px(Vec(54.0, 42.002)), module,
                                           AyysKing::OPENHATPUSH_PARAM));
    addChild(createLightCentered<LEDBezelLight<WhiteLight>>(
        mm2px(Vec(54.0, 42.002)), module, AyysKing::OPENHAT_LIGHT));

    addParam(createParamCentered<LEDBezel>(mm2px(Vec(6.999, 84.999)), module,
                                           AyysKing::BONGO1PUSH_PARAM));
    addChild(createLightCentered<LEDBezelLight<WhiteLight>>(
        mm2px(Vec(6.999, 84.999)), module, AyysKing::BONGO1_LIGHT));

    addParam(createParamCentered<LEDBezel>(mm2px(Vec(18.75, 84.999)), module,
                                           AyysKing::BONGO2PUSH_PARAM));
    addChild(createLightCentered<LEDBezelLight<WhiteLight>>(
        mm2px(Vec(18.75, 84.999)), module, AyysKing::BONGO2_LIGHT));

    addParam(createParamCentered<LEDBezel>(mm2px(Vec(30.501, 84.999)), module,
                                           AyysKing::BONGO3PUSH_PARAM));
    addChild(createLightCentered<LEDBezelLight<WhiteLight>>(
        mm2px(Vec(30.501, 84.999)), module, AyysKing::BONGO3_LIGHT));

    addParam(createParamCentered<LEDBezel>(mm2px(Vec(42.249, 84.999)), module,
                                           AyysKing::CLAVEPUSH_PARAM));
    addChild(createLightCentered<LEDBezelLight<WhiteLight>>(
        mm2px(Vec(42.249, 84.999)), module, AyysKing::CLAVE_LIGHT));

    addParam(createParamCentered<LEDBezel>(mm2px(Vec(54.0, 84.999)), module,
                                           AyysKing::CYMPUSH_PARAM));
    addChild(createLightCentered<LEDBezelLight<WhiteLight>>(
        mm2px(Vec(54.0, 84.999)), module, AyysKing::CYMBAL_LIGHT));

    addInput(createInputCentered<PJ301MPort>(mm2px(Vec(11.24, 30.801)), module,
                                             AyysKing::LENGTHCVIN_INPUT));
    addInput(createInputCentered<PJ301MPort>(mm2px(Vec(31.249, 30.801)), module,
                                             AyysKing::SPEEDCVIN_INPUT));
    addInput(createInputCentered<PJ301MPort>(mm2px(Vec(6.999, 56.0)), module,
                                             AyysKing::KICKTRIGIN_INPUT));
    addInput(createInputCentered<PJ301MPort>(mm2px(Vec(18.75, 56.0)), module,
                                             AyysKing::SNARE1TRIGIN_INPUT));
    addInput(createInputCentered<PJ301MPort>(mm2px(Vec(30.501, 56.0)), module,
                                             AyysKing::SNARE2TRIGIN_INPUT));
    addInput(createInputCentered<PJ301MPort>(mm2px(Vec(42.249, 56.0)), module,
                                             AyysKing::CLOSEDHATTRIGIN_INPUT));
    addInput(createInputCentered<PJ301MPort>(mm2px(Vec(54.0, 56.0)), module,
                                             AyysKing::OPENHATTRIGIN_INPUT));
    addInput(createInputCentered<PJ301MPort>(mm2px(Vec(6.999, 99.001)), module,
                                             AyysKing::BONGO1TRIGIN_INPUT));
    addInput(createInputCentered<PJ301MPort>(mm2px(Vec(18.75, 99.001)), module,
                                             AyysKing::BONGO2TRIGIN_INPUT));
    addInput(createInputCentered<PJ301MPort>(mm2px(Vec(30.501, 99.001)), module,
                                             AyysKing::BONGO3TRIGIN_INPUT));
    addInput(createInputCentered<PJ301MPort>(mm2px(Vec(42.249, 99.001)), module,
                                             AyysKing::CLAVETRIGIN_INPUT));
    addInput(createInputCentered<PJ301MPort>(mm2px(Vec(54.0, 99.001)), module,
                                             AyysKing::CYMTRIGIN_INPUT));

    addOutput(createOutputCentered<PJ301MPort>(mm2px(Vec(49.731, 30.801)),
                                               module, AyysKing::SUM_OUTPUT));
    addOutput(createOutputCentered<PJ301MPort>(
        mm2px(Vec(6.999, 70.002)), module, AyysKing::KICKOUT_OUTPUT));
    addOutput(createOutputCentered<PJ301MPort>(
        mm2px(Vec(18.75, 70.002)), module, AyysKing::SNARE1OUT_OUTPUT));
    addOutput(createOutputCentered<PJ301MPort>(
        mm2px(Vec(30.501, 70.002)), module, AyysKing::SNARE2OUT_OUTPUT));
    addOutput(createOutputCentered<PJ301MPort>(
        mm2px(Vec(42.249, 70.002)), module, AyysKing::CLOSEDHATOUT_OUTPUT));
    addOutput(createOutputCentered<PJ301MPort>(mm2px(Vec(54.0, 70.002)), module,
                                               AyysKing::OPENHATOUT_OUTPUT));
    addOutput(createOutputCentered<PJ301MPort>(
        mm2px(Vec(6.999, 112.999)), module, AyysKing::BONGO1OUT_OUTPUT));
    addOutput(createOutputCentered<PJ301MPort>(
        mm2px(Vec(18.75, 112.999)), module, AyysKing::BONGO2OUT_OUTPUT));
    addOutput(createOutputCentered<PJ301MPort>(
        mm2px(Vec(30.501, 112.999)), module, AyysKing::BONGO3OUT_OUTPUT));
    addOutput(createOutputCentered<PJ301MPort>(
        mm2px(Vec(42.249, 112.999)), module, AyysKing::CLAVEOUT_OUTPUT));
    addOutput(createOutputCentered<PJ301MPort>(
        mm2px(Vec(54.0, 112.999)), module, AyysKing::CYMOUT_OUTPUT));
  }
};
