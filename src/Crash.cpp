...
struct Crash : Module {
  ...
  enum LightId { CRASH_LIGHT, LIGHTS_LEN };
  ...
  void process(const ProcessArgs &args) override {
    ...
    CrashLightBrightness =
        std::max(0.f, CrashLightBrightness - (float)(args.sampleTime * 10.f));
    lights[CRASH_LIGHT].setBrightnessSmooth(CrashLightBrightness,
                                            args.sampleTime);
...
struct CrashWidget : ModuleWidget {
  CrashWidget(Crash *module) {
    setModule(module);
    setPanel(
        createPanel(asset::plugin(pluginInstance, "res/panels/Crash.svg")));
