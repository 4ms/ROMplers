...
struct Ride : Module {
  ...
  enum LightId { RIDE_LIGHT, LIGHTS_LEN };
  ...
  void process(const ProcessArgs &args) override {
    ...
    if (trigRising || buttonRising) {
      ...
      if (inputs[SAMPLECVIN_INPUT].isConnected()) {
        ...
        sampleIndex +=
            (int)round(cv * (numSamples / 10.f)); // ±5V → ±numSamples/2
      }
