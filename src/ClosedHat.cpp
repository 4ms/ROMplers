...
struct ClosedHat : Module {
  ...
  enum LightId { CLOSEDHAT_LIGHT, LIGHTS_LEN };
  ...
  int getSampleLengthByIndex(int index) {
    switch (index) {
    case 0:
      return ClosedHat1_len;
    case 1:
      return ClosedHat2_len;
    case 2:
      return ClosedHat3_len;
    case 3:
      return ClosedHat4_len;
    case 4:
      return ClosedHat5_len;
    case 5:
      return ClosedHat6_len;
    case 6:
      return ClosedHat7_len;
    case 7:
      return ClosedHat8_len;
    case 8:
      return ClosedHat9_len;
    case 9:
      return ClosedHat10_len;
    case 10:
      return ClosedHat11_len;
    case 11:
      return ClosedHat12_len;
    case 12:
      return ClosedHat13_len;
    case 13:
      return ClosedHat14_len;
    case 14:
      return ClosedHat15_len;
    case 15:
      return ClosedHat16_len;
    case 16:
    default:
      return 0;
    }
  ...
  void process(const ProcessArgs &args) override {
    ...
    const float sampleCV = inputs[SAMPLECVIN_INPUT].isConnected()
                               ? inputs[SAMPLECVIN_INPUT].getVoltage() / 5.f
                               : 0.f; // -1..1
    const float pitchCV = inputs[PITCHCVIN_INPUT].isConnected()
                              ? inputs[PITCHCVIN_INPUT].getVoltage() / 5.f
                              : 0.f; // -1..1
    const float decayCV = inputs[DECAYCVIN_INPUT].isConnected()
                              ? inputs[DECAYCVIN_INPUT].getVoltage() / 5.f
                              : 0.f; // -1..1
    const float volCV =
    ...
    if (triggered) {
      ...
      float sampleMod =
          sampleKnob +
          ((sampleCV > 0 ? (numSamples - 1 - sampleKnob) : sampleKnob) *
           sampleCV);
      ...
      float decayMod =
          decayKnob + ((decayCV > 0 ? (1.f - decayKnob) : decayKnob) * decayCV);
      ...
      float pitchMod =
          pitchKnob +
          ((pitchCV > 0 ? (1.f - pitchKnob) : (1.f + pitchKnob)) * pitchCV);
      pitchMod = std::clamp(pitchMod, -1.f, 1.f);

      float normalizedPitch = (pitchMod + 1.f) * 0.5f;
      pitchRatio = MIN_PLAYBACK_SPEED +
                   normalizedPitch * (MAX_PLAYBACK_SPEED - MIN_PLAYBACK_SPEED);
      pitchRatio *= 44100.f / args.sampleRate;
    }

    // Light decay
    ClosedHatLightBrightness = std::max(
        0.f, ClosedHatLightBrightness - (float)(args.sampleTime * 10.f));
    lights[CLOSEDHAT_LIGHT].setBrightnessSmooth(ClosedHatLightBrightness,
                                                args.sampleTime);
    ...
    if (playing && currentSample) {
      ...
      if (idx >= numSamplesInt) {
        ...
        int16_t s1s =
            (int16_t)(samplePtr[idx * 2] | (samplePtr[idx * 2 + 1] << 8));
        int16_t s2s = (int16_t)(samplePtr[nextIdx * 2] |
                                (samplePtr[nextIdx * 2 + 1] << 8));
...
struct ClosedHatWidget : ModuleWidget {
  ClosedHatWidget(ClosedHat *module) {
    setModule(module);
    setPanel(
        createPanel(asset::plugin(pluginInstance, "res/panels/ClosedHat.svg")));
