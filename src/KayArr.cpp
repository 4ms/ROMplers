...
struct KayArr : Module {
  ...
  KayArr() {
    ...
    configSwitch(RIMSHOTPUSH_PARAM, 0.f, 1.f, 0.f, "Rimshot Trig",
                 {"Off", "On"});
    configSwitch(COWBELLPUSH_PARAM, 0.f, 1.f, 0.f, "Cowbell Trig",
                 {"Off", "On"});
    ...
    static const struct {
      ...
      int output;
      int light;
    } voiceMap[10] = {{KRKick, sizeof(KRKick), KICKTRIGIN_INPUT, KICKPUSH_PARAM,
                       KICKOUT_OUTPUT, KICK_LIGHT},
                      {KRSnare, sizeof(KRSnare), SNARETRIGIN_INPUT,
                       SNAREPUSH_PARAM, SNAREOUT_OUTPUT, SNARE_LIGHT},
                      {KRTom, sizeof(KRTom), TOMTRIG_INPUT, TOMPUSH_PARAM,
                       TOMOUT_OUTPUT, TOM_LIGHT},
                      {KRClosedHat, sizeof(KRClosedHat), CLTRIG_INPUT,
                       CLPUSH_PARAM, CLOUT_OUTPUT, CL_LIGHT},
                      {KROpenHat, sizeof(KROpenHat), OHTRIG_INPUT, OHPUSH_PARAM,
                       OHOUT_OUTPUT, OH_LIGHT},
                      {KRClave, sizeof(KRClave), CLAVETRIG_INPUT,
                       CLAVEPUSH_PARAM, CLAVEOUT_OUTPUT, CLAVE_LIGHT},
                      {KRRimshot, sizeof(KRRimshot), RIMSHOTTRIG_INPUT,
                       RIMSHOTPUSH_PARAM, RIMSHOTOUT_OUTPUT, RIMSHOT_LIGHT},
                      {KRCowbell, sizeof(KRCowbell), COWBELLTRIG_INPUT,
                       COWBELLPUSH_PARAM, COWBELLOUT_OUTPUT, COWBELL_LIGHT},
                      {KRCymbal, sizeof(KRCymbal), CYMBALTRIG_INPUT,
                       CYMPUSH_PARAM, CYMBALOUT_OUTPUT, CYMBAL_LIGHT},
                      {KRConga, sizeof(KRConga), CONGATRIG_INPUT,
                       CONGAPUSH_PARAM, CONGAOUT_OUTPUT, CONGA_LIGHT}};
  ...
  void process(const ProcessArgs &args) override {
    // --- Speed parameter ---
    float speedKnobV = (params[SPEED_PARAM].getValue() + 1.f) * 2.5f;
    float speedCVV =
        inputs[SPEEDCVIN_INPUT].isConnected()
            ? std::clamp(inputs[SPEEDCVIN_INPUT].getVoltage(), -10.f, 10.f) *
                  0.5f
            : 0.f;
    float normSpeed = std::clamp(speedKnobV + speedCVV, 0.f, 5.f) / 5.f;
    float speed = SPEED_LOW + normSpeed * (SPEED_HIGH - SPEED_LOW);

    // --- Length parameter ---
    float lengthKnobV = params[LENGTH_PARAM].getValue() * 5.f;
    float lengthCVV =
        inputs[LENGTHCVIN_INPUT].isConnected()
            ? std::clamp(inputs[LENGTHCVIN_INPUT].getVoltage(), -10.f, 10.f) *
                  0.5f
            : 0.f;
    float normLength = std::clamp(lengthKnobV + lengthCVV, 0.f, 5.f) / 5.f;
    ...
    for (int i = 0; i < 10; i++) {
      ...
      bool retrigger = inputRising || buttonRising;
...
struct KayArrWidget : ModuleWidget {
  KayArrWidget(KayArr *module) {
    setModule(module);
    setPanel(
        createPanel(asset::plugin(pluginInstance, "res/panels/KayArr.svg")));
