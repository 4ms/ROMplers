...
struct KayArr : Module {
	enum ParamId {
		SPEED_PARAM, LENGTH_PARAM, MAINVOL_PARAM,
	...
	enum InputId {
		SPEEDCVIN_INPUT, LENGTHCVIN_INPUT,
	...
	enum OutputId {
		...
		CONGAOUT_OUTPUT, SUM_OUTPUT, OUTPUTS_LEN
	};
	enum LightId {
	...
	void process(const ProcessArgs& args) override {
		...
		float speedCVV = inputs[SPEEDCVIN_INPUT].isConnected() ? std::clamp(inputs[SPEEDCVIN_INPUT].getVoltage(), -10.f, 10.f) * 0.5f : 0.f;
		...
		float lengthCVV = inputs[LENGTHCVIN_INPUT].isConnected() ? std::clamp(inputs[LENGTHCVIN_INPUT].getVoltage(), -10.f, 10.f) * 0.5f : 0.f;
...
struct KayArrWidget : ModuleWidget {
	KayArrWidget(KayArr* module) {
		...
		addParam(createParamCentered<Davies1900hBlack>(mm2px(Vec(49.731, 15.501)), module, KayArr::MAINVOL_PARAM));
		...
		addOutput(createOutputCentered<PJ301MPort>(mm2px(Vec(49.731, 30.801)), module, KayArr::SUM_OUTPUT));
