...
struct OpenHat : Module {
  ...
  enum LightId { OPENHAT_LIGHT, LIGHTS_LEN };
  ...
  int getSampleLengthByIndex(int index) {
    ...
    case 8:
      return OpenHat9_len;
    case 9:
      return OpenHat10_len;
    case 10:
      return OpenHat11_len;
    case 11:
      return OpenHat12_len;
    case 12:
      return OpenHat13_len;
    case 13:
      return OpenHat14_len;
    case 14:
      return OpenHat15_len;
    case 15:
      return OpenHat16_len;
    default:
      return 0;
    }
  }
  ...
  void process(const ProcessArgs &args) override {
    ...
    if (trigRising || buttonRising) {
      ...
      float sampleCV = inputs[SAMPLECVIN_INPUT].isConnected()
    ...
    if (OpenHatLightBrightness < 0.f)
      OpenHatLightBrightness = 0.f;
    ...
    if (playing && currentSample) {
      ...
      float pitchRatio =
          MIN_PLAYBACK_SPEED +
          (pitchMod + 1.f) * 0.5f * (MAX_PLAYBACK_SPEED - MIN_PLAYBACK_SPEED);
      ...
      if (idx >= numSamps) {
        ...
        float decayMod =
            decayKnob +
            ((decayCV > 0 ? (1.f - decayKnob) : decayKnob) * decayCV);
        decayMod = std::clamp(decayMod, 0.f, 1.f);

        const float decayTime =
            0.005f + decayMod * ((float)numSamps / 44100.f - 0.005f);
    ...
    float volume =
        inputs[VOLCVIN_INPUT].isConnected()
            ? std::clamp(inputs[VOLCVIN_INPUT].getVoltage() / 5.f, 0.f, 1.f)
            : 1.f;
...
struct OpenHatWidget : ModuleWidget {
  OpenHatWidget(OpenHat *module) {
    setModule(module);
    setPanel(
        createPanel(asset::plugin(pluginInstance, "res/panels/OpenHat.svg")));
