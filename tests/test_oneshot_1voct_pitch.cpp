...
TEST_CASE("Slap: Unison(1), 1/12 V -> one semitone up") {
	CHECK(calc1VperOctPitchRatio(slapTotalVolts(1.f, 1.f / 12.f)) == doctest::Approx(std::pow(2.f, 1.f / 12.f)));
}
...
TEST_CASE("OrchHits: Unison(2), 1/12 V -> one semitone up") {
	CHECK(calc1VperOctPitchRatio(orchTotalVolts(2.f, 1.f / 12.f)) == doctest::Approx(std::pow(2.f, 1.f / 12.f)));
}
